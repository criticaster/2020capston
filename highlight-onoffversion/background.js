                        ...
                        if(is_new_word){
                            ...
                            chrome.storage.sync.set({'word_list': word_list}, function(){
                                var notifOptions = {
                                    ...
                                    iconUrl:'icon48.png',
                                    title: "highlight", 
                                    message : data[0] + "가 단어목록에 추가되었습니다.",
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse){
    ...
    if (checked == false){
        chrome.storage.sync.get(['tooltip_option'], function(userOptions){
            if (userOptions.tooltip_option ==  undefined || userOptions.tooltip_option == "on"){
                show_tooltip = true;
            }
            else{
                show_tooltip = false;
    ...
    if(request.todo == "searchThis") {
        if(show_tooltip == true){
            ...
            sendResponse({res:searchForTooltip("NO_SEARCH_RESULT")});
        }
    }
    ...
    if(request.todo == "backgroundUpdate"){
        if(request.opt == "off"){
